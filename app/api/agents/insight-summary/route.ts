      ...
      if (riskAgent) {
        try {
          ...
          if (riskResult.success && riskResult.data) {
            ...
            logger.info('[InsightSummary] ✅ RiskAgent completed:', {
              riskScore: riskAnalysis.totalRisk,
              volatility: riskAnalysis.volatility,
              sentiment: riskAnalysis.marketSentiment,
            });
          ...
          logger.warn('[InsightSummary] RiskAgent failed, continuing:', err);
        }
      }
      ...
      if (hedgingAgent) {
        ...
        for (const symbol of tokensToHedge) {
          try {
            ...
            logger.warn(`[InsightSummary] HedgingAgent failed for ${symbol}:`, err);
          }
        }
...
Return ONLY valid JSON.`,
            ...
            try {
              ...
              logger.warn('[InsightSummary] Could not parse LeadAgent synthesis JSON');
            }
          } catch {
            logger.warn('[InsightSummary] LLM unavailable for LeadAgent synthesis');
          }
          ...
          logger.warn('[InsightSummary] LeadAgent failed, continuing:', err);
        }
      }
      ...
      // ====================================================================
      logger.warn('[InsightSummary] Agent pipeline failed, using fallback:', orchestratorError);
