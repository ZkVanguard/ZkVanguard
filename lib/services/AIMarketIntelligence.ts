...
import {
  ...
  type FiveMinBTCSignal,
  type FiveMinSignalHistory,
} from './market-data/Polymarket5MinService';
import { DelphiMarketService, type PredictionMarket } from './market-data/DelphiMarketService';
import {
  MultiAssetSignalService,
  type MultiAssetSignal,
} from './market-data/MultiAssetSignalService';
...
export class AIMarketIntelligence {
  ...
  static async getMarketContext(assets?: string[]): Promise<AIMarketContext> {
    ...
    const { fetchBroadCryptoMarkets, summarize: summarizeBroad } =
      await import('./market-data/PolymarketBroadMarketsService');
    ...
    const fusionTargets = Array.from(
      new Set([
        'BTC',
        'ETH',
    ...
    const [
      ...
      Polymarket5MinService.getLatest5MinSignal().catch((err) => {
        logger.warn('[AIMarketIntelligence] 5min signal fetch failed', { error: err });
        return null;
      }),
      DelphiMarketService.getRelevantMarkets(assets).catch((err) => {
        logger.warn('[AIMarketIntelligence] Predictions fetch failed', { error: err });
        return [];
      }),
      this.fetchPriceData(assets).catch((err) => {
        logger.warn('[AIMarketIntelligence] Price data fetch failed', { error: err });
        return {};
      }),
      MultiAssetSignalService.getAggregatedSentiment(multiAssetTargets).catch((err) => {
        ...
        return {
          ...
          strongCount: 0,
          avgConfidence: 0,
          perAsset: {} as Record<string, MultiAssetSignal | null>,
        };
      }),
    ...
    const broadAsPredictions: PredictionMarket[] = broadAll
      ...
      .map((b) => ({
        id: `broad-${b.id}`,
        question: b.question,
        category: (b.marketType === 'event'
          ? 'event'
          : b.marketType === 'price-target' || b.marketType === 'price-range'
            ? 'price'
            : b.marketType === 'binary-direction'
              ? 'price'
              : 'market') as PredictionMarket['category'],
        probability: b.probability,
        volume: `$${(b.volume24hr / 1000).toFixed(1)}K 24h`,
        impact: (b.volume24hr > 100_000
          ? 'HIGH'
          : b.volume24hr > 10_000
            ? 'MODERATE'
            : 'LOW') as PredictionMarket['impact'],
    ...
    const context: AIMarketContext = {
      ...
      manifoldMarkets,
      marketSentiment: this.calculateSentiment(
        fiveMinSignal,
        signalHistory,
        mergedPredictions,
        priceData
    ...
    logger.debug(`AI Market Context generated in ${duration}ms`, {
      component: 'AIMarketIntelligence',
    });
  ...
  private static analyzeCorrelation(
    ...
    const directions = [btcChange, ethChange, croChange, suiChange].map((c) =>
      c > 0 ? 1 : c < 0 ? -1 : 0
    );
    ...
    if (btcChange > 0) {
      if (ethChange > 0) alignedAssets.push('ETH');
      else divergingAssets.push('ETH');
      if (croChange > 0) alignedAssets.push('CRO');
      else divergingAssets.push('CRO');
      if (suiChange > 0) alignedAssets.push('SUI');
      else divergingAssets.push('SUI');
    } else {
      if (ethChange < 0) alignedAssets.push('ETH');
      else divergingAssets.push('ETH');
      if (croChange < 0) alignedAssets.push('CRO');
    ...
    const correlationBoost =
      marketAlignment > 80 ? 15 : marketAlignment > 60 ? 10 : marketAlignment > 40 ? 5 : 0;
  ...
  private static detectRiskCascade(
    ...
    const highImpactNegative = predictions.filter(
      (p) =>
        p.impact === 'HIGH' &&
        p.probability > 60 &&
  ...
  private static analyzeLiquidity(
    ...
    const liquidityConfidencePenalty =
      liquidityRatio < 0.001 ? -15 : liquidityRatio < 0.01 ? -10 : liquidityRatio < 0.1 ? -5 : 0;
  ...
  private static enhancePredictions(
    ...
    return predictions.map((pred, _index) => {
      // Calculate AI relevance score
      const impactScore = pred.impact === 'HIGH' ? 30 : pred.impact === 'MODERATE' ? 20 : 10;
      const recScore =
        pred.recommendation === 'HEDGE' ? 40 : pred.recommendation === 'MONITOR' ? 20 : 5;
  ...
  private static calculateSentiment(
    ...
    const momentum =
      history.signals.length > 0 ? (recentUps / history.signals.length - 0.5) * 100 : 0;
  ...
  private static generateSummary(
    ...
    const urgency: 'HIGH' | 'MEDIUM' | 'LOW' =
      hedgeCount >= 2 || signal?.signalStrength === 'STRONG'
        ? 'HIGH'
        : hedgeCount >= 1 || confidence > 70
          ? 'MEDIUM'
          : 'LOW';
    ...
    if (history.streak.count >= 2)
      keyFactors.push(`${history.streak.count}-signal ${history.streak.direction} streak`);
...
export const getPredictionsForAgent =
  AIMarketIntelligence.getPredictionsForAgent.bind(AIMarketIntelligence);
