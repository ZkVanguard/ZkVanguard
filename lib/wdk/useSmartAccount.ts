...
export function useSmartAccount() {
  ...
  const depositWithGasless = useCallback(
    async (amount: string) => {
      setLoading(true);
      setError(null);
      try {
        ...
        if (!response.ok) {
          ...
          if (errorData.error === 'Insufficient USDT balance on Safe' && errorData.safeAddress) {
            const manualError = new Error(
              `Gasless account needs setup. Please transfer USDT to your Safe Address: ${errorData.safeAddress}`
            );
